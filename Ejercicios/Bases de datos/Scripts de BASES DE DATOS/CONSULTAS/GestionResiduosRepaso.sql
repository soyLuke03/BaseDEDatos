...
WHERE ciudad_empresa LIKE 'Huelva'
OR ciudad_empresa LIKE 'Malaga';
--2
...
FROM traslado t 
WHERE fecha_envio between to_date('1/12/1994','DD/MM/YYYY') AND to_date('31/12/1994','DD/MM/YYYY') ;
--5
SELECT cod_residuo, count(cod_constituyente)
FROM residuo_constituyente rc
GROUP BY cod_residuo;
--6
SELECT nvl(avg(cantidad),0)
...
--10
SELECT count(t.fecha_envio),e.ciudad_empresa,d.ciudad_destino
FROM traslado t , empresaproductora e ,destino d 
...
FROM empresatransportista e,traslado t,residuo_empresa re, empresaproductora e2,residuo_constituyente rc,residuo r,constituyente c 
...
--12
SELECT sum(t.kms)*t.coste coste_km
FROM traslado t , empresaproductora e 
WHERE t.nif_empresa = e.nif_empresa 
AND lower(e.nombre_empresa) LIKE 'Carbonsur'
GROUP BY t.coste;
--13
SELECT count(c.cod_constituyente),r.cod_residuo 
FROM constituyente c ,residuo_constituyente rc,residuo r 
WHERE c.cod_constituyente = rc.cod_constituyente 
AND rc.cod_constituyente = c.cod_constituyente
...
AND lower(e.nombre_empresa) LIKE '%e%'
