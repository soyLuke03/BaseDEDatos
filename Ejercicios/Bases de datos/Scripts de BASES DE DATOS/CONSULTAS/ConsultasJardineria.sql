...
representen a ningÃºn cliente.*/

SELECT e.nombre ||' '|| e.apellido1 nombre_completo, e.puesto 
FROM empleado e
...
/*4.Devuelve el nombre del producto del que se han vendido mÃ¡s unidades.
(Ten en cuenta que tendrÃ¡s que calcular cuÃ¡l es el nÃºmero total de unidades
que se han vendido de cada producto a partir de los datos de la tabla detalle_pedido.
Una vez que sepas cuÃ¡l es el cÃ³digo del producto, puedes obtener su nombre fÃ¡cilmente.)*/

SELECT lp.* 
FROM (SELECT p.nombre producto
...
SELECT e.nombre ||' '|| e.apellido1 ||' '|| e.apellido2 empleado, e.email 
FROM empleado e 
WHERE e.codigo_jefe = (SELECT e2.codigo_empleado
						FROM empleado e2
						WHERE upper(e2.nombre) LIKE '%ALBERTO%'
						AND upper(e2.apellido2) LIKE '%SORIA%');
						
					
--Consultas multitabla (composiciÃ³n interna)
...
SELECT DISTINCT nombre_cliente cliente, nombre, o.ciudad
...
SELECT e.nombre  empleado, e2.nombre "el jefe"
...
--10Devuelve un listado de las diferentes gamas de producto que ha comprado cada cliente.
...
-- Ejercicio 4:
SELECT p.estado , COUNT(p.codigo_pedido) num_pedidos
FROM pedido p
GROUP BY p.estado 
...
-- Ejercicio 8:
SELECT c.ciudad , COUNT(c.codigo_cliente) num_cliente
FROM cliente c  
