...
CREATE OR REPLACE PROCEDURE uno
AS
...
BEGIN 
	FOR registro IN c_empleado
		LOOP
			dbms_output.put_line(registro.nombre ||' '|| registro.apellidos ||' '|| registro.puesto);
		END LOOP;
END;
...
AS
	CURSOR c_empleado IS
	SELECT count(codigo_empleado) nEmpleados, codigo_oficina 
	FROM empleado
	GROUP BY codigo_oficina;
BEGIN
	FOR i IN c_empleado
		LOOP
			dbms_output.put_line('Hay: '||i.nEmpleados||' empleados en la oficina: '||i.codigo_oficina);
		END LOOP;
END;
...
CREATE OR REPLACE PROCEDURE tres (texto varchar2)
AS
	contador NUMBER :=0;
	CURSOR c_empleado IS
	SELECT c.codigo_cliente ||' | '|| c.nombre_cliente datos
	FROM cliente c 
	WHERE c.nombre_cliente LIKE '%'||texto||'%';
BEGIN 
	FOR i IN c_empleado
		LOOP
			contador:= contador +1;
			dbms_output.put_line('Nombre cliente: ' || i.datos);
		END LOOP;
	
	dbms_output.put_line(contador ||  ' clientes');
END;
...
CREATE OR REPLACE PROCEDURE cuatro
AS
	CURSOR c_producto IS
	SELECT datos FROM 
		(SELECT p.codigo_producto ||' Nombre: '|| p.nombre ||' Gama: '|| p.gama datos, count(dp.codigo_pedido)
		FROM producto p, detalle_pedido dp
		WHERE p.codigo_producto = dp.codigo_producto
		GROUP BY p.codigo_producto, p.nombre, p.gama
		ORDER BY count(dp.codigo_pedido) DESC) p
	WHERE rownum <=5;
BEGIN 
	FOR i IN c_producto
		LOOP
			dbms_output.put_line(i.datos);
		END LOOP;
END;
...
--medio actual.
CREATE OR REPLACE PROCEDURE cinqui
AS
	CURSOR c_limiteCredito IS
	SELECT c.nombre_cliente 
	FROM cliente c,
		(SELECT nvl(avg(c.limite_credito),0) mediaCredito
		FROM cliente c) l
	WHERE c.limite_credito < l.mediaCredito;
BEGIN 
	FOR i IN c_limiteCredito
		LOOP
			UPDATE cliente SET limite_Credito = limite_Credito + (limite_Credito*0.5)
			WHERE nombre_cliente IN   	(SELECT c.nombre_cliente 
										FROM cliente c,
											(SELECT nvl(avg(c.limite_credito),0) mediaCredito
											FROM cliente c) l
										WHERE c.limite_credito < l.mediaCredito);
...
CREATE OR REPLACE FUNCTION seis(codigo varchar2)
RETURN NUMBER
AS
	cantidad detalle_pedido.cantidad%TYPE:=0;
...
END;
	...
	SELECT cantidad_vendida ('11679') FROM dual;
...
CREATE OR REPLACE FUNCTION siete(f_codigo varchar2, f_fecha number)
RETURN NUMBER
AS
f_cantidad NUMBER:=0;
exc_no_data_found EXCEPTION;
...
CREATE OR REPLACE FUNCTION ocho(v_codcli varchar2)
RETURN NUMBER
AS
v_cantidad NUMBER;
BEGIN
	...
	v_cantidad :=0;
	SELECT count(codigo_pedido) INTO v_cantidad FROM pedido;
	DELETE FROM pedido WHERE codigo_cliente = v_codgocli;
	RETURN v_cantidad;
...
CREATE OR REPLACE PROCEDURE nueve (gamaCH varchar2)
AS
	CURSOR c_gama IS
	SELECT p.codigo_producto 
	FROM producto p
	WHERE p.gama LIKE '%'||gamaCH||'%';
	
	contadorProductos NUMBER :=0;
BEGIN
	FOR registro IN c_gama
		LOOP 
			UPDATE producto SET precio_venta = round(precio_venta + (precio_venta*0.7))
			WHERE gama like	(SELECT distinct p.gama
							FROM producto p
							WHERE p.gama LIKE '%'||gamaCH||'%') ;
			UPDATE producto SET precio_proveedor = round(precio_proveedor + (precio_proveedor*0.7))
			WHERE gama like	(SELECT DISTINCT p.gama
							FROM producto p
							WHERE p.gama LIKE '%'||gamaCH||'%') ;
	...
	dbms_output.put_line(contadorProductos||' productos actualizados');

	EXCEPTION 
	WHEN TO_MANY_ROWS THEN
	dbms_output.put_line('Error bro');
END;
...
--Nº total de clientes representados: ………….(4)
...
--? (4) Nº de clientes representados
